...
public class GameMaster {
    ...
    public Integer coinCollect = 0;
    public Integer itRemainToCollect = 9;
    ...
    public void renderFrame(Graphics graphics) {
        if(getPlayer().playerWin == false) {
            ...
            graphics.drawString("Coins already collect: " + coinCollect.toString(), 100, 20);
            graphics.drawString("It remains to collect: " + itRemainToCollect.toString(), 250, 20);
        }
        else {
            graphics.drawImage(victory, 180, 0, null);
        }
    ...
    public void deleteCoins(int x, int y){
        coinCollect++;
        itRemainToCollect--;
        if (itRemainToCollect == 0) exitEnable = true;
        this.gameObjects.removeIf(coin -> coin instanceof Coin && coin.getXPosition() == x &&
                coin.getYPosition() == y);
